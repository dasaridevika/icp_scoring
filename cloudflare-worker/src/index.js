/**
 * Production-Grade Cloudflare Worker AI - Ideal Customer Profile (ICP) Scoring Engine
 * Model: @cf/meta/llama-3.1-8b-instruct
 * Reference: Saber ICP Scoring Model Framework (https://www.saber.app/glossary/icp-scoring-model)
 * 
 * Dynamic zero-hardcoding evaluation:
 * - Firmographic Fit (30% weight)
 * - Technographic Fit (25% weight)
 * - Intent & Timing Signals (25% weight)
 * - Persona & Buying Authority (20% weight)
 */
...
Respond ONLY in pure valid JSON.`;
    ...
    // Smart Dynamic Fallback Parser if model returned incomplete JSON
    if (!aiResult) {
    ...
    // Mathematical Weighted Aggregation (Saber ICP Rubric: 30%, 25%, 25%, 20%)
    ...
    // Saber Tier Categorization & Sales Cadence
    ...
    // Saber Quality-Weighted Forecast Calculation
    ...
    return jsonResponse({
      ...
      pillar_scores: {
        ...
        firmographic: {
          score: fScore,
          weight: 0.30,
          points_contributed: Math.round(fScore * 0.30),
        ...
        persona: {
          score: pScore,
          weight: 0.20,
          points_contributed: Math.round(pScore * 0.20),
